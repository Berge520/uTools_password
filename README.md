...
- **二维码工具箱**：任意文字 / 网址 / WiFi 字符串生成 **QR 码 / Data Matrix**（复制或保存图片）；剪贴板 / 图片 / 截屏三种方式离线识别 QR 码与 Data Matrix（支持中文 UTF-8/GBK 自动转码、旋转图识别），结果可复制、在密码库中搜索或一键存为密码（搜索「二维码」「扫码」「数据矩阵」直达）
- **uTools 智能入口**：在 uTools 搜索框输入文字打开插件即自动搜索（插件内顶部子搜索栏，设置中可开关）；选中/复制网址可直接「查找密码」；拖拽 JSON/CSV 备份文件到 uTools 自动导入；剪贴板图片可直接「识别二维码密码」
- **版本检查**：设置页自动比对 GitHub 最新版本，有新版本一键跳转插件详情页更新
...
- **WebDAV 云备份**：可开关，把密码库备份到 WebDAV（坚果云 / Nextcloud / 群晖等），支持测试连接、立即备份、云端恢复；备份带 **SHA-256 完整性校验**（上传后回读校验，防静默损坏）、**多版本防覆盖**（时间戳版本文件，云端保留最近 3 份）、**恢复前预览**（先读摘要再覆盖）、**云端版本列表**（恢复时弹出版本选择）、**本地导出**（另存一份到本机）、常见状态码**中文友好提示**与失败**自动重试**
- **三套主题**：亮色 / 暗色 / 护眼（可记忆）
...
- **v0.2.0**：
  - **WebDAV 备份完整性校验**：备份体附带 SHA-256 校验和，上传后回读重新比对，防止静默写入损坏；恢复 / 预览前也校验，损坏文件会被拦截；
  - **多版本防覆盖**：每次备份写入带时间戳（精确到毫秒）的版本文件而非覆盖旧文件，云端自动保留最近 3 份并清理更早版本；
  - **恢复前预览**：从云端读取备份摘要（是否加密、条目数、备份时间）后再确认覆盖，避免误操作；
  - **云端版本列表**：恢复时弹出版本选择列表（文件名 + 备份时间），可任选一次历史备份恢复；
  - **本地导出**：弹窗把当前密码库另存为备份文件到本机，云端之外再留一份；
  - **更多安全细则**：预览时即校验完整性；备份成功时提示保留份数；明文字符未开启主密码时备份前再次警告；
  - **友好错误提示**：401 / 403 / 404 / 405 / 409 / 413 / 501 / 507 等常见 WebDAV 状态码映射为中文说明；
  - **失败自动重试**：网络级异常自动重试 3 次，HTTP 状态错误不重试以免无意义等待；
  - 修复：无备份 / 目录不存在等场景恢复逻辑更健壮。
- **v0.1.0**：
  - 新增**二维码工具箱**（搜索「二维码」「扫码」或工具菜单打开）：任意文字 / 网址 / WiFi 字符串生成 **QR 码 / Data Matrix**（复制 / 保存图片）；剪贴板 / 图片 / 截屏离线识别，支持 Data Matrix、中文 UTF-8/GBK 自动转码与旋转识别，结果可复制、搜索密码库或一键存为密码；
  - 新增 uTools **匹配指令**：选中/复制网址可直接「查找密码」（自动搜索该网址对应条目）；拖拽 JSON/CSV 备份文件到 uTools 自动触发导入；剪贴板有图片时可直接「识别二维码密码」（进入工具箱显示结果）；
  - 新增**快速搜索**：uTools 搜索框输入文字后打开插件即自动搜索，插件内顶部显示子搜索栏，设置中可开关；
  - 新增**版本检查**：设置页自动比对 GitHub 最新版本，「✓ 最新 / → 可更新」一键跳转 uTools 插件详情页；
  - 新增**联网 / 离线模式**：一键开关，离线模式自动停用版本检查、WebDAV 云备份、数据同步等所有联网功能，数据仅存本机；版本检查可在设置中独立开关；
  - 新增导出 **Chrome / Edge 可直接导入的 CSV**（当前分组或全部，含备注列，导出文件可往返导入）；
  - **大库性能优化**：密码列表**虚拟化渲染**，只渲染可视区卡片，实测 1000+ 条滚动 / 搜索 / 增删不再卡顿；同步数据改浅拷贝、加密复用已派生会话密钥，显著降低大库保存耗时与 CPU 占用；
  - **超限防崩溃**：写入超过 uTools 单键上限（约 1MB）时自动回退为仅本地文件存储并关闭云端同步，不再出现「doc max size 1M」报错崩溃；设置页同步区块新增超限与备份提示；
  - **自绘主题滚动条**：滚动条改用 JS 自绘滑块，随**亮色 / 暗色 / 护眼**三套主题自动变色、悬停高亮；修复 uTools 环境下原生 `::-webkit-scrollbar` 样式失效（纯白突兀、三主题恒同默认色）问题；
  - **界面美化**：主题背景径向渐变、选中文字随主题高亮、卡片 / 头像 / 输入框 hover 质感统一；
  - 按 uTools 官方文档优化 plugin.json：插件初始高度 660、功能指令精简为 4 个、新增 3 个匹配指令功能；
  - 修复：打开插件时命令词「我的密码」被误当成搜索词、设置页版本号硬编码显示错误等问题。
- **v0.0.6**：
